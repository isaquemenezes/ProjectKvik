<?php
    ...
    //Input Cidade
    if(isset($_POST['cidade'])){ $cidade=filter_input(INPUT_POST,'cidade',FILTER_SANITIZE_FULL_SPECIAL_CHARS);  }
    else{  $cidade=null; }

    //Input Bairro
    if(isset($_POST['bairro'])){ $bairro=filter_input(INPUT_POST,'bairro',FILTER_SANITIZE_FULL_SPECIAL_CHARS); }
    else{ $bairro=null; }

    //Input formulário de contato
    if(isset($_POST['contato'])){ $contato=filter_input(INPUT_POST,'contato', FILTER_SANITIZE_FULL_SPECIAL_CHARS); }
    else{ $contato=null; }
